  ...
  return (
    <main className="min-h-screen mx-auto max-w-[420px] px-5 py-8 pb-24 bg-[#0f172a]">
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <button type="button" onClick={() => setView((v) => addMonths(v.year, v.month, -1))} className="h-10 w-10 rounded-2xl bg-[#1e293b] border border-white/8 text-slate-300 flex items-center justify-center active:scale-[0.96] transition-transform" aria-label="Previous month">
          <span className="text-xl leading-none">‹</span>
        </button>
        <h2 className="text-base font-bold text-slate-100 tracking-tight">{monthLabel(view.year, view.month)}</h2>
        <button type="button" onClick={() => setView((v) => addMonths(v.year, v.month, 1))} disabled={isCurrentMonth} className="h-10 w-10 rounded-2xl bg-[#1e293b] border border-white/8 text-slate-300 flex items-center justify-center active:scale-[0.96] transition-transform disabled:opacity-30 disabled:active:scale-100" aria-label="Next month">
          <span className="text-xl leading-none">›</span>
        </button>
      </div>
      ...
      <div className="rounded-2xl bg-[#1e293b] shadow-card border border-white/5 overflow-hidden p-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendar.days.length > 0 && (() => {
            ...
            const pads = Array.from({ length: pad }, (_, i) => <div key={`pad-${i}`} className="aspect-square" />);
            return pads.concat(calendar.days.map((day) => {
              const isToday = day.date === todayKey;
              const href = day.date === todayKey ? '/today' : `/today?date=${day.date}`;
              return (
                <Link key={day.date} href={href}
                  className={`aspect-square rounded-xl flex items-center justify-center text-xs font-semibold transition-all active:scale-[0.95] ${
                    isToday ? 'ring-2 ring-[#22c55e] ring-offset-2 ring-offset-[#1e293b] bg-[#22c55e]/10 text-[#22c55e]'
                    : day.full ? 'bg-[#22c55e] text-white'
                    : day.completed > 0 ? 'bg-[#22c55e]/20 text-[#22c55e]'
