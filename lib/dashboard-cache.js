...
/**
 * Bust cache for a specific date (called after habits change so the next
 * dashboard load fetches fresh active-only habits from the server).
 */
export function clearCachedDashboard(date) {
  L1.delete(date);
  if (typeof window === 'undefined') return;
  try { localStorage.removeItem(LS + date); } catch { /* ignore */ }
}
